...
import { Droplet, Zap, Recycle, BarChart, DollarSign, FileText, CheckCircle, Activity, TrendingUp, PieChart, FileCheck, Info, AlertTriangle, ArrowRight, ArrowLeft } from 'lucide-react';
...
const App = () => {
  ...
  const calculateResults = () => {
    ...
    const currentTotalDailyCost = (volumeNum * currentWaterCostNum) + (energyTargetNum * currentEnergyCostNum); // Example current energy use *might* differ, use target as proxy
    const wepaAnnualSavings = (currentTotalDailyCost * 365) - wepaTotalAnnualFee;
    ...
    // More sophisticated calculation would factor in energy source replaced, grid intensity, specific process emissions etc.
    const carbonReductionKgPerDay = (energyTargetNum * 0.4) + (volumeNum * 0.0001); // Simplified: kWh * grid factor + water treatment factor
    ...
    setResults({
      ...
      wepaAnnualSavings,
      wepaNpv,
      wepaWaterPrice: wepaWaterPriceNum, // Store for display
      wepaEnergyPrice: wepaEnergyPriceNum, // Store for display
  ...
  const formatCurrency = (num, decimals = 0) => {
      ...
      // Handle very large or small numbers that might otherwise show as $0 or -$0
      if (Math.abs(value) < 0.01 && value !== 0) {
           options.minimumFractionDigits = 2;
           options.maximumFractionDigits = 4; // Show more precision if very small
      }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <div className="relative mt-1 rounded-md shadow-sm">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2 rounded-md border border-gray-300 focus:ring-teal-500 focus:border-teal-500 sm:text-sm"
              placeholder="e.g., 400,000"
              required
              min="1"
            />
             <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                <span className="text-gray-500 sm:text-sm">gallons/day</span>
             </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[ { name: 'bod', label: 'BOD', placeholder: 'e.g., 1500' },
               { name: 'cod', label: 'COD', placeholder: 'e.g., 3000' },
               { name: 'tss', label: 'TSS', placeholder: 'e.g., 500' } ].map(char => (
                <div key={char.name}>
                    ...
                    <input
                        ...
                        onChange={handleChange}
                        className="block w-full px-3 py-2 rounded-md border border-gray-300 focus:ring-teal-500 focus:border-teal-500 sm:text-sm"
      ...
      <div className="flex justify-end mt-8">
        <button
          ...
          className="px-6 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
        >
    ...
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="relative">
          ...
          <div className="relative mt-1 rounded-md shadow-sm">
            ...
            <input
              ...
              required
              min="0"
              max={formData.wastewaterVolume || undefined} // Can't produce more than influent
            />
             <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                <span className="text-gray-500 sm:text-sm">gallons/day</span>
             </div>
          </div>
           {formData.wastewaterVolume && formData.cleanWaterTarget > formData.wastewaterVolume && (
              <p className="mt-1 text-xs text-red-600">Cannot target more clean water than daily wastewater volume.</p>
            )}
        </div>
      ...
      <div className="flex justify-between mt-8">
        <button
          type="button"
          onClick={prevStep}
          className="px-6 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 flex items-center"
        >
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Financial Preference <span className="text-red-500">*</span>
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {/* Direct Buy Option Card */}
             <div
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-full ${formData.financialPreference === 'direct' ? 'bg-teal-100' : 'bg-gray-100'}`}>
                    <DollarSign size={20} className="text-teal-600" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-800">Direct Buy</h3>
                    <p className="text-sm text-gray-500">Purchase the system outright (CAPEX).</p>
                  </div>
                </div>
             </div>
              {/* WEPA Option Card */}
             ...
             >
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-full ${formData.financialPreference === 'wepa' ? 'bg-blue-100' : 'bg-gray-100'}`}>
                     <Activity size={20} className="text-blue-600" />
                  </div>
                  <div>
                      <h3 className="font-semibold text-gray-800">WEPA</h3>
                      <p className="text-sm text-gray-500">Pay per unit of water/energy (No CAPEX).</p>
                  </div>
                </div>
             </div>
          </div>
        ...
        {formData.financialPreference === 'wepa' && (
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 space-y-3 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="wepaWaterPrice" className="block text-sm text-gray-600 mb-1">
                  Price per gallon of treated water <span className="text-red-500">*</span>
                </label>
                <div className="relative rounded-md shadow-sm">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className="block w-full pl-7 pr-12 py-2 rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    ...
                    />
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        <span className="text-gray-500 sm:text-sm">/ gallon</span>
                    </div>
                </div>
              </div>
              <div>
                <label htmlFor="wepaEnergyPrice" className="block text-sm text-gray-600 mb-1">
                  Price per kWh of energy generated <span className="text-red-500">*</span>
                </label>
                 <div className="relative rounded-md shadow-sm">
                     <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span className="text-gray-500 sm:text-sm">$</span>
        ...
        <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="currentWaterCost" className="block text-sm text-gray-600 mb-1">
                Current cost per gallon of water <span className="text-red-500">*</span>
              </label>
              <div className="relative rounded-md shadow-sm">
                 <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="text-gray-500 sm:text-sm">$</span>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="block w-full pl-7 pr-12 py-2 rounded-md border border-gray-300 focus:ring-teal-500 focus:border-teal-500 sm:text-sm"
            ...
            <div>
              <label htmlFor="currentEnergyCost" className="block text-sm text-gray-600 mb-1">
                Current cost per kWh of energy <span className="text-red-500">*</span>
              </label>
              <div className="relative rounded-md shadow-sm">
                 <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="text-gray-500 sm:text-sm">$</span>
      ...
      <div className="flex justify-between mt-8">
        ...
        <button
          ...
          className="px-6 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
        >
          <FileCheck size={18} className="mr-2" /> Generate Proposal
        </button>
      </div>
    </div>
  );
  ...
  const renderResults = () => {
    ...
    const recommendation = () => {
        ...
        const directBuyBetter = results.directBuyNpv > results.wepaNpv;
        const preferDirect = formData.financialPreference === 'direct';
        const preferWepa = formData.financialPreference === 'wepa';
    ...
    // Placeholder Chart Component (Still used for some items)
    const PlaceholderChart = ({ title, description, children }) => (
        <div className="bg-white p-4 rounded shadow-sm border border-gray-200 h-full flex flex-col">
            ...
            <p className="text-xs text-gray-500 mb-3 flex-grow">{description}</p>
            <div className="chart-placeholder min-h-[80px] bg-gray-100 rounded flex items-center justify-center text-gray-400 text-sm italic p-2">
      ...
      <div className="bg-white rounded-lg shadow-xl overflow-hidden animate-fadeIn">
        ...
        <div className="bg-gradient-to-r from-teal-600 to-blue-500 px-6 py-5">
          <h2 className="text-3xl font-bold text-white flex items-center">
            <FileCheck size={32} className="mr-3" />
            Customized Wastewater Recovery Proposal
          </h2>
        ...
        <div className="p-6 md:p-8 space-y-8">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <p className="text-gray-700 text-sm leading-relaxed">
              This proposal outlines a Cambrian Innovation solution designed to process approximately <span className="font-medium">{formatNumber(results.wastewaterVolume)}</span> gallons of wastewater per day.
              Based on your inputs (BOD: {results.bod} mg/L, COD: {results.cod} mg/L, TSS: {results.tss} mg/L) and desired outputs, the recommended primary technology is the <span className="font-semibold">{results.primaryTechnology}</span>.
              <br /><em>Reasoning: {results.technologyReasoning}</em>
              {results.additionalTechNote && <><br /><span className="text-indigo-700"><Info size={14} className="inline mr-1" /> {results.additionalTechNote}</span></>}
              The system aims to produce <span className="font-medium">{formatNumber(results.cleanWaterOutput)}</span> gallons/day of clean water for reuse and generate <span className="font-medium">{formatNumber(results.energyOutput)}</span> kWh/day of renewable energy.
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-4 rounded shadow-sm border border-gray-100 space-y-2">
                        <h5 className="font-semibold text-gray-800 flex items-center">
                            <DollarSign size={18} className="text-teal-600 mr-2" /> Direct Buy Option
                        </h5>
                        <div className="text-sm space-y-1">
                            <p><span className="text-gray-500 w-28 inline-block">Est. CAPEX:</span> <span className="font-medium">{formatCurrency(results.baseCapex)}</span></p>
                            <p><span className="text-gray-500 w-28 inline-block">Est. Annual OPEX:</span> <span className="font-medium">{formatCurrency(results.opexAnnual)}</span></p>
                            <p><span className="text-gray-500 w-28 inline-block">Annual Savings:</span> <span className="font-medium text-green-700">{formatCurrency(results.directBuyAnnualSavings)}</span></p>
                            <p><span className="text-gray-500 w-28 inline-block">Payback Period:</span> <span className="font-medium">~{results.paybackMonths}</span></p>
                            <p><span className="text-gray-500 w-28 inline-block">10-Year NPV:</span> <span className="font-medium">{formatCurrency(results.directBuyNpv)}</span></p>
                        </div>
                    </div>
                    {/* WEPA Summary */}
                    <div className="bg-white p-4 rounded shadow-sm border border-gray-100 space-y-2">
                       <h5 className="font-semibold text-gray-800 flex items-center">
                            <Activity size={18} className="text-blue-600 mr-2" /> WEPA Option
                        </h5>
                        <div className="text-sm space-y-1">
                            <p><span className="text-gray-500 w-28 inline-block">Est. CAPEX:</span> <span className="font-medium">{formatCurrency(0)} (Financed)</span></p>
                             <p><span className="text-gray-500 w-28 inline-block">Est. Monthly Fee:</span> <span className="font-medium">{formatCurrency(results.wepaMonthlyFee)}</span></p>
                             <p><span className="text-gray-500 w-28 inline-block">Annual Savings:</span> <span className="font-medium text-green-700">{formatCurrency(results.wepaAnnualSavings)}</span></p>
                            <p><span className="text-gray-500 w-28 inline-block">10-Year NPV:</span> <span className="font-medium">{formatCurrency(results.wepaNpv)}</span></p>
                             <p className="text-xs text-gray-500 pt-1">(Based on WEPA rates: {formatCurrency(results.wepaWaterPrice, 3)}/gal, {formatCurrency(results.wepaEnergyPrice, 2)}/kWh)</p>
                        </div>
                    </div>
                </div>
           </div>

            {/* 3. Operational Capabilities & Financial Visualizations - UPDATED */}
            <div className="space-y-6">
                 <h4 className="text-lg font-semibold text-gray-800 mb-1">Operational & Financial Visualizations</h4>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">

                     {/* UPDATED: Annual Savings Comparison */}
                     <div className="bg-white p-4 rounded shadow-sm border border-gray-200 text-center col-span-1 md:col-span-2 lg:col-span-1 h-full flex flex-col justify-center">
                         ...
                         <div className="flex justify-around items-center flex-grow mt-2">
                             <div className="text-center">
                                 <div className="text-3xl font-bold text-green-700">{formatCurrency(results.directBuyAnnualSavings)}</div>
                                 <div className="text-xs text-gray-500">Direct Buy / Year</div>
                             </div>
                             <div className="text-center">
                                 <div className="text-3xl font-bold text-emerald-700">{formatCurrency(results.wepaAnnualSavings)}</div>
                                 <div className="text-xs text-gray-500">WEPA / Year</div>
                             </div>
                         </div>
                     ...
                     {/* Payback Period (Using EconomicHighlight Component) */}
                     <EconomicHighlight
                        title="Payback Period (Direct Buy)"
                        // Conditionally add " months" if the value is a number, otherwise show the original 'N/A...' string
                        value={isFinite(results.paybackPeriodYears) ? `${results.paybackMonths} months` : results.paybackMonths}
                        colorClass="text-teal-600"
                        // Keep the 'unit' prop to display years below, for context
                        unit={isFinite(results.paybackPeriodYears) ? `~${formatNumber(results.paybackPeriodYears, 1)} Years` : null}
                        note={!isFinite(results.paybackPeriodYears) ? "(Savings may not cover OPEX or CAPEX)" : null}
                      />


                     {/* UPDATED: 10-Year NPV Comparison */}
                     <div className="bg-white p-4 rounded shadow-sm border border-gray-200 text-center col-span-1 md:col-span-2 lg:col-span-1 h-full flex flex-col justify-center">
                         <h4 className="font-medium text-gray-700 mb-2 text-sm">10-Year NPV Comparison</h4>
                         <div className="flex justify-around items-center flex-grow mt-2">
                              <div className="text-center">
                                 <div className="text-3xl font-bold text-teal-600">{formatCurrency(results.directBuyNpv)}</div>
                                 <div className="text-xs text-gray-500">Direct Buy NPV</div>
                             </div>
                             <div className="text-center">
                                 <div className="text-3xl font-bold text-blue-600">{formatCurrency(results.wepaNpv)}</div>
                                 <div className="text-xs text-gray-500">WEPA NPV</div>
                             </div>
                         </div>
                     ...
                     {/* Water Treatment Efficiency (Using EconomicHighlight Component) */}
                     <EconomicHighlight
                        title="Water Treatment Efficiency"
                        value={`${formatNumber(results.waterTreatmentEfficiency, 1)}%`}
                        colorClass="text-blue-600"
                        unit={`(${formatNumber(results.cleanWaterOutput)} / ${formatNumber(results.wastewaterVolume)} gal/day)`}
                        note="Targeted clean water reuse"
                     />


                     {/* Energy Generation (Using EconomicHighlight Component) */}
                     <EconomicHighlight
                        title="Daily Energy Generation"
                        value={formatNumber(results.energyOutput)}
                        colorClass="text-yellow-600"
                        unit="kWh / day"
                        note="Targeted renewable energy"
                      />


                     {/* Environmental Impact (Using EconomicHighlight Component) */}
                      <EconomicHighlight
                        title="Environmental Impact"
                        value={formatNumber(results.carbonReductionKgPerDay)}
                        colorClass="text-green-600"
                        unit="kg CO₂ eq. / day"
                        note="Estimated reduction (simplified)"
                      />
                 </div>
            </div>
          ...
          {/* 4. Recommendation - UNCHANGED */}
           <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              ...
              <p className="text-gray-700 text-sm leading-relaxed">
                {recommendation()} Consider your organization's capital availability and risk tolerance when making the final decision.
              </p>
           </div>
          ...
          <div className="pt-4 border-t mt-6">
            ...
            <p className="text-xs text-gray-500 italic leading-relaxed">
              The financial figures (CAPEX, OPEX, savings, NPV, payback) presented in this proposal are <span className="font-semibold">preliminary estimates</span>.
              {/*The base CAPEX is anchored to the reference cost of <span className="font-semibold">{formatCurrency(10000000)} for a 400,000 gallon per day EcoVolt Reactor system</span> ({formatCurrency(CAPEX_PER_GALLON_DAILY_ECOVOLT)} per gallon of daily capacity) and scaled linearly based on your provided daily wastewater volume.*/}
              <br />Actual project costs will vary based on:
              <ul className="list-disc list-inside ml-4 my-1">
                  <li>The final selection and combination of technologies (EcoVolt, Bio Viper, BlueCycle MBR, filtration, digestion, etc.).</li>
                  <li>Specific wastewater characteristics beyond BOD, COD, TSS.</li>
                  <li>Required effluent quality and specific reuse applications.</li>
                  <li>Site-specific conditions (geotechnical, existing infrastructure, utilities, space constraints).</li>
                  <li>Permitting requirements and local regulations.</li>
              </ul>
             <span className="font-semibold">A detailed engineering study and consultation with Cambrian Innovation are required for a formal quote and precise system design.</span> These estimates are provided for initial planning and comparison purposes only.
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
           <button
            onClick={() => { setFormSubmitted(false); setResults(null); }} // Reset results when editing
            className="px-5 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 text-sm font-medium flex items-center"
          >
             <ArrowLeft size={16} className="mr-2" /> Edit Inputs
          </button>
           <p className="text-xs text-gray-500 hidden sm:block">Generated: {new Date().toLocaleString()}</p>
          <button
            onClick={() => window.print()}
            className="px-5 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 text-sm font-medium flex items-center"
          >
            <FileText size={16} className="mr-2" /> Export / Print Proposal
          </button>
        </div>
      </div>
    );
      ...
      <header className="bg-gradient-to-r from-teal-700 to-blue-700 text-white shadow-lg sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs md:text-sm text-teal-200 hidden sm:block">Wastewater Economic Simulator</div>
          </div>
        </div>
      </header>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="mb-8 px-2">
                 <ol className="flex items-center w-full text-sm font-medium text-center text-gray-500">
                    {[ { step: 1, label: "Wastewater" },
                       { step: 2, label: "Outputs" },
                       { step: 3, label: "Financials" } ].map((item, index) => (
                        <li key={item.step} className={`flex md:w-full items-center ${ currentStep >= item.step ? 'text-teal-600' : ''} ${ index !== 2 ? "after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 " + (currentStep > item.step ? 'after:border-teal-100' : '') : ''}`}>
                            <span className={`flex items-center justify-center w-8 h-8 rounded-full shrink-0 mr-2 ${ currentStep >= item.step ? 'bg-teal-100' : 'bg-gray-100' }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               ...
               <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                 <div className="bg-teal-50 px-4 py-3 border-b border-teal-100">
                   <h3 className="text-lg font-semibold text-teal-800 flex items-center"><Zap size={20} className="mr-2 text-yellow-500" /> EcoVolt Reactor</h3>
                 </div>
                 <div className="p-4 text-sm text-gray-600 space-y-2">
                   <p>Converts wastewater's organic load (high BOD/COD) directly into biogas for renewable energy generation using electrogenic microbes.</p>
                   <ul className="list-disc list-inside text-xs pl-2 space-y-1">
                       <li>Ideal for high-strength industrial wastewater.</li>
                       <li>Produces valuable biogas (methane).</li>
                       <li>Reduces sludge production significantly.</li>
                       <li>Can operate as a standalone or pre-treatment step.</li>
                   </ul>
                 </div>
               </div>
                {/* Bio Viper Card */}
               <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                 <div className="bg-indigo-50 px-4 py-3 border-b border-indigo-100">
                   <h3 className="text-lg font-semibold text-indigo-800 flex items-center"><Droplet size={20} className="mr-2 text-indigo-500" /> Bio Viper</h3>
                 </div>
                 <div className="p-4 text-sm text-gray-600 space-y-2">
                   <p>An integrated fixed-film activated sludge (IFAS) system for robust BOD/TSS removal with enhanced nutrient reduction capabilities.</p>
                    <ul className="list-disc list-inside text-xs pl-2 space-y-1">
                       <li>Handles moderate to high loads and fluctuations.</li>
                       <li>Compact footprint compared to conventional activated sludge.</li>
                       <li>Reduced sludge yield.</li>
                        <li>Suitable for direct discharge or pre-treatment for reuse.</li>
                   </ul>
                 </div>
               </div>
                {/* BlueCycle MBR Card */}
                <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                 <div className="bg-blue-50 px-4 py-3 border-b border-blue-100">
                   <h3 className="text-lg font-semibold text-blue-800 flex items-center"><Recycle size={20} className="mr-2 text-blue-500" /> BlueCycle MBR</h3>
                 </div>
                 <div className="p-4 text-sm text-gray-600 space-y-2">
                   <p>A Membrane Bioreactor (MBR) providing the highest quality effluent, ideal for direct water reuse applications by removing &gt;99% BOD/TSS.</p>
                   <ul className="list-disc list-inside text-xs pl-2 space-y-1">
                       <li>Produces Title 22 quality water (or similar standards).</li>
                       <li>Physical barrier ensures consistent high quality.</li>
                       <li>Very compact footprint.</li>
                       <li>Often used as a final polishing step after EcoVolt or Bio Viper.</li>
                   </ul>
                 </div>
               </div>
            </div>
           ...
           {/* Footer content */}
             <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left">
               ...
               <div className="text-sm">
                 <p className="text-gray-400">15 Main Street, Suite 318, Watertown, MA 02472</p>
                 <p className="text-teal-400 mt-1">(617) 307-1755 | info@cambrianinnovation.com</p>
               </div>
             </div>
          <div className="border-t border-gray-700 mt-6 pt-6 text-center text-gray-500 text-xs">
            &copy; {new Date().getFullYear()} Cambrian Innovation Inc. All Rights Reserved. Proposal estimates are non-binding.
          </div>
        </div>
      </footer>
        ...
        <style jsx global>{`
            ...
            /* Basic print styling */
             @media print {
                 ...
                 header, footer, button, form, .no-print { display: none !important; }
                 main { margin: 0 !important; padding: 0 !important; }
                 .shadow-xl, .shadow-lg, .shadow-md { box-shadow: none !important; }
                 .bg-gradient-to-r { background: #0D9488 !important; /* Fallback solid color */ } /* Example: Use a solid color for gradients */
                  .p-6, .p-8 { padding: 1rem !important; } /* Adjust padding for print */
                 .container { max-width: 100% !important; }
                 .grid { display: block !important; } /* Simplify grid for printing */
